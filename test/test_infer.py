...
def load_llaisys_model(model_path, device_name, quant_dtype=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="/root/autodl-tmp/大模型计算_HW3/llaisys-thu/DeepSeek-R1-Distill-Qwen-1.5B", type=str)
    ...
    parser.add_argument("--quant", choices=["int8", "float8", "none"], default="none", 
                       help="Quantization type: int8, float8, or none (default: none)")

    args = parser.parse_args()
    
    # Convert quantization string to torch dtype
    quant_dtype = None
    if args.quant == "int8":
        quant_dtype = torch.int8
    elif args.quant == "float8":
    ...
    model = load_llaisys_model(model_path, args.device, quant_dtype=quant_dtype)
    start_time = time.time()
